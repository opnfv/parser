...
register_parser_image() {
    openstack ${debug} image list | tee | grep -qwo "${PARSER_IMAGE_NAME}" && {
...
create_parser_user_and_project() {
    ...
    openstack ${debug} project list | tee | grep -qwo "${PARSER_PROJECT}" && {
    ...
    openstack ${debug} user list | tee | grep -qwo ${PARSER_USER} && {
...
translator_and_deploy_vRNC() {
    ...
    (
        ...
        openstack ${debug} stack list | tee |  grep -qow ${PARSER_STACK_NAME} && {
        ...
        openstack ${debug} stack show ${PARSER_STACK_NAME} | tee | grep -qow "CREATE_COMPLETE" && {
...
reset_parser_test() {
    ...
    (
        ...
        parser_image_id=$(openstack ${debug} image list | tee | grep -w "${PARSER_IMAGE_NAME}" | awk '{print $2}')
    ...
    openstack ${debug} user list | tee | grep -qow "${PARSER_USER}" && {
